...
test.describe("Authentication", () => {
  ...
  test("login redirects to dashboard", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { level: 1, name: /tổng quan|phòng/i })).toBeVisible();
  });

  test("invalid credentials show error", async ({ page }) => {
    await page.goto("/login");
    await page.getByLabel(/email/i).fill("not-a-user@example.com");
    await page.getByRole("textbox", { name: /mật khẩu|password/i }).fill("wrong-password-123");
    await page.getByRole("button", { name: /đăng nhập|log in/i }).click();
